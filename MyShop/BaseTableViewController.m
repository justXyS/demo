...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    if(alertView.tag == 101){
        UINavigationController *vcr = [[UIStoryboard storyboardWithName:@"Login" bundle:nil] instantiateViewControllerWithIdentifier:@"login"];
        LoginViewController *vt = vcr.viewControllers[0];
        vt.loginname = [[NSUserDefaults standardUserDefaults] objectForKey:@"loginname"];
